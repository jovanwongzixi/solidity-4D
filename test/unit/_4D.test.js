    ...
    : describe("_4D", function(){
        ...
        describe("enter4D", function(){
            it("Fails if bet amount is too low", async() => {
                await expect(_4D.enter4D(1000, { value: 0 })).to.be.revertedWith("_4D__BetAmountTooLow")
            })

            it("Fails if bet amount is too high", async() => {
                await expect(_4D.enter4D(1000, { value: ethers.utils.parseEther("1")})).to.be.revertedWith("_4D__BetAmountTooHigh")
            })
